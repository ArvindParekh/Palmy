...
interface LabClientPageProps {
  folders: Prisma.Palmlet_FolderGetPayload<{
    include: {
      palmlets: true
    }
  }>[]
...
export default function LabClientPage({ folders, userId }: LabClientPageProps) {
  ...
  const handleSave = async () => {
    ...
    try {
      let targetFolderId = saveFolder

      // If creating a new folder, create it first
      if (isCreatingNewFolder && newFolderName.trim()) {
        const folderResult = await createPalmletFolder({
          userId,
          folderName: newFolderName.trim(),
          folderDescription: `Folder for ${generatedTemplate.category}`
        })

        if (!folderResult.success) {
          toast.error(folderResult.message)
          setIsSaving(false)
          return
        }
      ...
      // Save the palmlet to the folder
      const result = await createNewPalmlet(
        targetFolderId,
      ...
      if (result.success) {
        ...
        // Optionally redirect to the palmlet editor
        // router.push(`/palmlets/${targetFolderId}/editor/${result.data.id}`)
      } else {
                    ...
                    <DialogContent>
                      ...
                      <div className="space-y-4">
                            ...
                            <div className="space-y-2">
                              {folders.length > 0 ? (
                                folders.map((folder) => (
                                  <Button
                                    key={folder.id}
                                    variant={saveFolder === folder.id ? "default" : "outline"}
                                ...
                                <p className="text-sm text-muted-foreground">No folders available. Create a new one!</p>
                              )}
                            </div>
                          )}
                        </div>
                        ...
                        <div className="flex gap-2">
                          <Button
                            onClick={handleSave}
                            disabled={(!saveFolder && !newFolderName.trim()) || isSaving}
                            className="flex-1"
                          >
                            {isSaving ? (
                              <>
