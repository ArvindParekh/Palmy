...
import { getFolderPalmlets } from "@/lib/data/palmlet";
...
export default async function FolderPage({
  ...
  if (!folderPalmlets.success) {
    return (
      <div className="w-full min-h-screen flex items-center justify-center">
        <div className="text-center">
          <h1 className="text-2xl font-bold text-gray-900 dark:text-white mb-2">Error</h1>
          <p className="text-gray-600 dark:text-gray-400 mb-4">{folderPalmlets.message}</p>
          <Link href="/palmlets">
          ...
          <div className="flex items-center space-x-4 mb-8">
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">
                  {folderInfo.folderDescription || 'No description provided'}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-8">
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-2xl p-6 border border-white/50 dark:border-slate-600/40">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">{palmlets.length}</p>
              ...
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
                    {Array.from(new Set(palmlets.flatMap(p => p.tags.map(tag => tag.tagName)))).length}
                  </p>
                  <p className="text-xs text-gray-600 dark:text-gray-400">Unique Tags</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-2xl p-6 border border-white/50 dark:border-slate-600/40">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
                    {palmlets.reduce((total, p) => total + p.variables.length, 0)}
                  </p>
                  <p className="text-xs text-gray-600 dark:text-gray-400">Total Variables</p>
                </div>
              </div>
            </div>
            ...
            <div className="bg-white/80 dark:bg-slate-900/80 backdrop-blur-md rounded-2xl p-6 border border-white/50 dark:border-slate-600/40">
              <div className="flex items-center space-x-3">
                ...
                <div>
                  <p className="text-2xl font-bold text-gray-900 dark:text-white">
                    {Math.round((palmlets.filter(p => p.content && p.content.length > 0).length / palmlets.length) * 100) || 0}%
                  </p>
