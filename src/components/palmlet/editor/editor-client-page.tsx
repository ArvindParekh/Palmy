...
export default function EditorPage({
   ...
   id: string;
   folderNumber: string;
   templateData: Prisma.PalmletGetPayload<{
      include: {
         tags: true;
         variables: true;
      };
   ...
   const handleSave = async () => {
      ...
      if (id === "") {
         const result = await createNewPalmlet(
            ...
            title,
            content || "",
            templateData.tags.map((t) => t.tagName),
            variables
         );
         if (result.success) {
         ...
         <div className='flex flex-col h-screen w-full bg-background text-foreground'>
            ...
            <header className='flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 p-4 border-b border-border shrink-0'>
               <div className='flex items-center gap-2 sm:gap-4 min-w-0 flex-1'>
                  ...
                  <div className='min-w-0 flex-1'>
                     ...
                     <p className='text-xs sm:text-sm text-muted-foreground px-2'>
                        Last saved {formatDistanceToNow(templateData.updatedAt)}{" "}
                        ago
                     </p>
                  </div>
               </div>
               <div className='flex items-center gap-2 sm:gap-4 shrink-0'>
                  ...
                  <Button
                     variant='outline'
                     size='sm'
                     className='hidden sm:inline-flex text-muted-foreground border-border hover:bg-accent hover:text-accent-foreground'
                  >
                     Share
                  </Button>
                  <Button
                     onClick={handleSave}
                     size='sm'
                     className='bg-foreground text-background hover:bg-foreground/90 shadow-lg hover:scale-105 transition-all duration-300'
                  >
            ...
            <div className='flex-1 flex min-h-0 relative'>
                                 ...
                                 .map((part, index) => {
                                    ...
                                    if (match) {
                                       return (
                                          <span
                                             key={index}
                                             className='bg-orange-500 text-foreground p-0.5 rounded-sm shadow-sm'
                                          >
                                             {match[1]}
                                          </span>
                                       );
               ...
               {isSidebarOpen && (
                  ...
                  >
                     <aside
                        className='absolute right-0 top-0 h-full w-[90vw] max-w-sm bg-background border-l border-border flex flex-col'
                        ...
                        >
                           ...
                           >
                              <div className='space-y-4'>
                                 ...
                                 <p className='text-xs text-muted-foreground'>
                                    Generate, rewrite, or improve your template
                                    with AI.
                                 </p>
                                 <Button
                                    variant='outline'
